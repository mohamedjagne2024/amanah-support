...
export default function Import() {
  ...
  const handleUploadFile = useCallback(() => {
    ...
    setIsUploading(true);
    router.post("/subcategories/import/upload", formData, {
      onSuccess: () => {
        setCurrentStep(1);
      },
      onError: (errors) => {
  ...
  const handleConfirmImport = useCallback(() => {
    setIsProcessing(true);
    router.post(
      "/subcategories/import/execute",
      { mappings: fieldMappings },
      {
        onSuccess: () => {
          router.visit("/subcategories");
        },
        onFinish: () => {
          setIsProcessing(false);
        }
  ...
  const isCategoryMapped = Object.values(fieldMappings).includes("category");
  const isAllRequiredMapped = isNameMapped && isCategoryMapped;

  const steps = ["Upload File", "Map Fields", "Preview & Import"];

  return (
    <AppLayout>
      <PageMeta title="Import SubCategories" />
      <main className="max-w-4xl pb-8">
        <PageBreadcrumb 
          title="Import SubCategories" 
          subtitle="SubCategories"
          subtitleUrl="/subcategories"
        />
              ...
              <div className="card-body space-y-5">
                ...
                <div className="rounded-lg bg-default-50 border border-default-200 p-4 mt-3">
                  ...
                  <ul className="text-sm text-default-600 space-y-1">
                    ...
                    <li>• Category names must match existing records exactly</li>
                    <li>• Duplicate subcategory names within the same category will be skipped</li>
                  </ul>
                </div>
                ...
                <div className="flex items-center justify-between p-4 border border-default-200 rounded-lg mt-3">
                  ...
                  <a href="/templates/subcategory-import-template.xlsx" download>
                ...
                <div className={`mb-5 p-3 rounded-lg flex items-center gap-2 ${
                  ...
                  {isAllRequiredMapped ? (
                    <>
                      ...
                      <span className="text-sm font-medium text-success">All required fields mapped - ready to continue</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-sm font-medium text-warning">
                        ...
                        {!isNameMapped && !isCategoryMapped && <span>,</span>}
                        {!isCategoryMapped && <span className="ml-1">"Category"</span>}
                      </span>
                    </>
                  )}
                </div>
                      ...
                      {props.previewData.preview_records.slice(0, 10).map((record, index) => (
                        <tr key={index} className="text-default-800">
                          ...
                          <td className="py-2.5 px-3.5 text-sm text-default-600">
                            {record.data.category || <span className="text-default-400 italic">No category provided</span>}
                          </td>
                          <td className="py-2.5 px-3.5">
              ...
              <div className="card-footer">
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      Import {props.previewData.valid_count} SubCategor{props.previewData.valid_count !== 1 ? 'ies' : 'y'}
                    </>
                  )}
                </button>
              </div>
